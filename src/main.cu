...
int main(int argc, const char* argv[]) {
	...
	NeuralNetwork nn = NeuralNetwork(0.025f);
    //TODO: build your network structure
		LinearLayer inp_layer = LinearLayer("linear_inp_layer", Shape(3,3));
		nn.addLayer(&inp_layer);
		nn.addLayer(new SigmoidActivation("sigmoid_1"));
		LinearLayer out_layer = LinearLayer("linear_out", Shape(3,1));
		nn.addLayer(&out_layer);
